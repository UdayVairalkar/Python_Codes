# Python_Codes
in this repository there areeee all python codes from basic to advance 
in this  we have all code of followig topic 
1st we have basic codes
number data type ip/op 
and it also have condion statement 
also it includes the arthmatic and relational and logical membership operator codes 
and many codes are in processing that i yet to be uploaded
ALL CODES HEEERE
